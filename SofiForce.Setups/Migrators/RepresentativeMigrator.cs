...
{
    public class RepresentativeMigrator
    {
        ...
        public static string SP_Representatives = @"
 select    e.site   as BranchCode,
		   e.PERSONNELNUMBER as RepresentativeCode,
		   ltrim(rtrim((replace(d.NAME,',',' '))))  as RepresentativeName,
		   1 as IsActive,
		   '' as title

from
  [SOF-SRV-DB12].[SofDynAXLive].dbo.HcmWorker             AS e   left join
  [SOF-SRV-DB12].[SofDynAXLive].dbo.DIRPARTYTABLE         AS d  ON e.PERSON = d.RECID 

--where d.MODIFIEDDATETIME>=DATEADD(DAY,-1,GETDATE())
";


        public RepresentativeMigrator( IConfiguration configuration)
        {
        ...
        public MigrationDTO Migrate()
        {
            ...
            try
            {
                ...
                _TempCommand.CommandText = "truncate table Representatives";
                _TempCommand.ExecuteNonQuery();

                _AXCommand.CommandType = CommandType.Text;
                _AXCommand.CommandText = SP_Representatives;
                ...
                // Migrrate Data
                _SqlBulkCopy.DestinationTableName = "Representatives";
                ...
                _SqlBulkCopy.ColumnMappings.Add("title", "title");
                ...
                _TempCommand.CommandText = "Migrate_Representatives";
                _TempCommand.ExecuteNonQuery();

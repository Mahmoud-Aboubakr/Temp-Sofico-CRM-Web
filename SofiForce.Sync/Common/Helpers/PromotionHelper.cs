...
namespace SofiForce.Promotion.Helpers
{
    public static class PromotionHelper
    {
        ...
        public static async Task<List<OrderPromotionFinal>> getOrderPromotions(SalesOrderModelMobile model)
        {
        ...
        public static Task<bool> IsAllowedRepeats(int? RepeatTypeId,int? ClientId, int? PromotionId,int? maxRepeats)
        {
            ...
            try
            {
                ...
                if (RepeatTypeId == 1)
                {
                    Repeats = new Criteria<BOPromotionOrderHistory>()
                                   .Add(Expression.Eq(nameof(BOPromotionOrderHistory.PromotionId), PromotionId))
                                   .Add(Expression.Eq(nameof(BOPromotionOrderHistory.ClientId), ClientId))
                                   .Count();
                }
                if (RepeatTypeId == 2)
                {
                    Repeats = new Criteria<BOPromotionOrderHistory>()
                                   .Add(Expression.Eq(nameof(BOPromotionOrderHistory.PromotionId), PromotionId))
                                   .Add(Expression.Eq(nameof(BOPromotionOrderHistory.ClientId), ClientId))
                                   .Add(Expression.Gt(nameof(BOPromotionOrderHistory.InvoiceDate), DateTime.Now.AddYears(-1), dateformatter))
                                   .Count();
                }
                if (RepeatTypeId == 3)
                {
                    Repeats = new Criteria<BOPromotionOrderHistory>()
                               .Add(Expression.Eq(nameof(BOPromotionOrderHistory.PromotionId), PromotionId))
                               .Add(Expression.Eq(nameof(BOPromotionOrderHistory.ClientId), ClientId))
                               .Add(Expression.Gt(nameof(BOPromotionOrderHistory.InvoiceDate), DateTime.Now.AddMonths(-1), dateformatter))
                               .Count();
                }
                if (RepeatTypeId == 4)
                {
                    Repeats = new Criteria<BOPromotionOrderHistory>()
                                   .Add(Expression.Eq(nameof(BOPromotionOrderHistory.PromotionId), PromotionId))
                                   .Add(Expression.Eq(nameof(BOPromotionOrderHistory.ClientId), ClientId))
                                   .Add(Expression.Gt(nameof(BOPromotionOrderHistory.InvoiceDate), DateTime.Now.AddDays(-1), dateformatter))
                                   .Count();
                }

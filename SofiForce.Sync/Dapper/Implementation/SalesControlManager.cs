...
namespace SofiForce.Sync.Dapper.Implementation
{
                ...
                using (var connection = _context.CreateConnection())
                {
                    ...
                    if (Totals.Count > 0)
                    {
                        ...
                        if (model.TargetValue > 0)
                            model.SalesPercentage = Math.Round(model.SalesValue / (decimal)model.TargetValue * 100, 0);

                        if (model.TargetVisit > 0)
                            model.VisitPercentage = Math.Round(model.VisitAll / (decimal)model.TargetVisit * 100, 0);

                        if (model.TargetCall > 0)
                            model.CallPercentage = Math.Round(model.VisitAll / (decimal)model.TargetCall * 100, 0);
                    ...
                    foreach (var item in model.SalesControlDetails)
                    {
                        ...
                        if (item.TargetValue > 0)
                            item.SalesPercentage = Math.Round(item.SalesValue / (decimal)item.TargetValue * 100, 0);

                        if (item.TargetVisit > 0)
                            item.VisitPercentage = Math.Round(item.VisitAll / (decimal)item.TargetVisit * 100, 0);

                        if (item.TargetCall > 0)
                            item.CallPercentage = Math.Round(item.VisitAll / (decimal)item.TargetCall * 100, 0);
            ...
            try
            {
                using (var connection = _context.CreateConnection())
                {
                    ...
                    foreach (var item in model)
                    {
                        ...
                        if (item.TargetValue > 0)
                            item.SalesPercentage = Math.Round(item.RouteSales / item.TargetValue * 100, 0);

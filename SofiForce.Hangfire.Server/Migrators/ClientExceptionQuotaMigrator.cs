...
{
    public class ClientExceptionQuotaMigrator
    {
        ...
        @"
Select
		  CustomerAccount  as ClientCode
		, ItemId as ItemCode
		, FromDate as FromDate
		, ToDate as ToDate
		, Qty as QTY

From [SOF-SRV-DB12].[SofDynAXLive].dbo.SFC_CustomerSalesLimitException ss
Where Operational   = 1
and ss.FROMDATE<=cast(GETDATE() as date) and ss.TODATE>=cast(GETDATE() as date)
Order By FromDate Desc
        ";
        public ClientExceptionQuotaMigrator( IConfiguration configuration)
        {
        ...
        public void Migrate()
        {
            ...
            try
            {
                ...
                _AXCommand.CommandText = SP_ClientExceptionQuota;

...
namespace SFFService
{
    public class WarehouseWorker : BackgroundService
    {
        ...
        {
            _logger.LogInformation("Warehouse Start at: {time}", DateTimeOffset.Now);
            try
            {
                _logger.LogInformation("Warehouse {error}: {time}", "Connections Done", DateTimeOffset.Now);
            }
            catch (Exception ex)
            {
                _logger.LogError("Warehouse {error}: {time}", ex.Message, DateTimeOffset.Now);
            }
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    {
                        _logger.LogError("Warehouse {error}: {time}", storeItemRes.Message, DateTimeOffset.Now);
                        continue;
        ...
        {
            _logger.LogInformation("Warehouse Stoped at: {time}", DateTimeOffset.Now);

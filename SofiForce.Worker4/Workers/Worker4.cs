...
namespace SofiForce.Worker
{
    public class Worker4 : BackgroundService
    {
        ...
        public Worker4(ILogger<Worker4> logger, IConfiguration configuration, IOrderService orderService)
        {
            _logger = logger;
        ...
        {
            //should be this
            //_logger.LogInformation("Worker4 Start at: {time}", DateTimeOffset.Now);
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    var worker = new Criteria<BOServiceWorker>()
                        .Add(Expression.Eq(nameof(BOServiceWorker.ServiceWorkerCode), "4"))
                ...
                {
                    //should be this
                    //_logger.LogError("Worker 4 {error}: {time}",ex.Message, DateTimeOffset.Now);
        ...
        {
            //should be this
            //_logger.LogInformation("Worker4 Stoped at: {time}", DateTimeOffset.Now);

...
@Component({
  ...
  templateUrl: './chooser-product.component.html',
  styleUrls: ['./chooser-product.component.scss']
})
export class ChooserProductComponent implements OnInit {
  ...
  constructor(
    ...
    if (this.config.data) {
      ...
      if(this.config.data.canChange){
        this.canChange= this.config.data.canChange;
      }
    ...
    var found=localStorage.getItem("chooser-product.component.termBy")
    if(found){
      console.log(found);
  ...
  OnTermChange(arg){
    localStorage.setItem("chooser-product.component.termBy",arg.value);
  }

...
@Component({
  ...
  templateUrl: './visit-activites.component.html',
  styleUrls: ['./visit-activites.component.scss']
})
export class VisitActivitesComponent implements OnInit {
  ...
  async manage(mode) {
    ...
    if (mode == 'x') {
      ...
      await (this._ClientActivityService.Export(this.searchModel)).subscribe((data: any) => {
        ...
        a.download = "Visits_"+new Date().toISOString().split('.')[0].replace(/[^\d]/gi,'')

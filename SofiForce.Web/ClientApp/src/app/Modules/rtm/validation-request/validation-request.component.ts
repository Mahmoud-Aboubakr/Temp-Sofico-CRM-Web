...
export class ValidationRequestComponent implements OnInit {
  ...
  constructor(
    ...
    this._translateService.get('Represetitive Tracking').subscribe((res) => { this.TRACKING_REPRESENTITIVE = res });

    this.searchModel.operationTypeId = 2;
  ...
  ngOnInit(): void {
    ...
    this.menuItems = [
      ...
      },
      {
        ...
        command: (e) => this.manage('tracking')
      },
      {
        label: 'Export',
  ...
  async manage(operation) {
    if (operation == 'create') {
      var ref = this.dialogService.open(ManageValidationRequestComponent, {
    ...
    if (operation == 'clients') {
      if(this.selected &&this.selected.operationId>0){
        var ref = this.dialogService.open(ManageValidationListComponent, {
          data:{operationId:this.selected.operationId},
    ...
    if(operation=='tracking'){
      if(this.selected &&this.selected.operationId>0){
        var ref = this.dialogService.open(TrackingGpsDetailComponent, {
          data:{representativeId:this.selected.representativeId,mode:2},
          header: this.TRACKING_REPRESENTITIVE,
          width: '90%',
          height:'90%',
          contentStyle: { "overflow": "auto" },
          baseZIndex: 10000

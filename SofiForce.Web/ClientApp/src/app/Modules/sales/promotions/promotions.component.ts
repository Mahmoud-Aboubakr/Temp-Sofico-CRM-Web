...
export class PromotionsComponent implements OnInit {
  ...
  async manage(mode) {
    ...
    if (mode == 'n') {
      ...
      var ref = this.dialogService.open(ManagePromotionComponent, {
        ...
        data: { promotionId: 0 ,promotionCategoryId:2},
        width: '900px',
    ...
    if (mode == 'v') {
      ...
      var ref = this.dialogService.open(ManagePromotionComponent, {
        header: this.EDIT_HEADER,
        data: { promotionId: this.selected.promotionId, editMode: 'v',promotionCategoryId:2 },
        width: '900px',
        contentStyle: { "max-height": "700px", "height": "700px", "overflow": "auto" },
        baseZIndex: 10000
      });
    ...
    if (mode == 'e') {
      if (this.selected != null && this.selected.promotionId > 0) {
        var ref = this.dialogService.open(ManagePromotionComponent, {
          header: this.EDIT_HEADER,
          data: { promotionId: this.selected.promotionId,promotionCategoryId:2 },
          width: '900px',
          contentStyle: { "max-height": "700px", "height": "700px", "overflow": "auto" },
          baseZIndex: 10000
        });
  ...
  async choose_Vendor() {
    ...
    ref.onClose.subscribe((vendor: VendorListModel) => {
      ...
      if (vendor) {
        this.searchModel.vendorCode = vendor.vendorCode;
      }

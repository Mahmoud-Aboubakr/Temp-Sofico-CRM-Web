...
@Component({
  selector: 'app-order-cash-all',
  templateUrl: './order-cash-all.component.html',
  styleUrls: ['./order-cash-all.component.scss']
})
export class OrderCashAllComponent implements OnInit {
  ...
  constructor(
    ...
    var found=localStorage.getItem("order-cash-all.component.termBy")
    if(found){
      console.log(found);
  ...
  OnTermChange(arg){
    localStorage.setItem("order-cash-all.component.termBy",arg.value);
  }
      ...
      if (this.selected != null && this.selected.salesId > 0) {
        this.confirmationService.confirm({
          ...
          accept: async () => {
            ...
            this._SalesOrderService.ClearCash(model).then(res => {
              this.advancedFilter();
              this.isLoading = false;
  ...
  async saveOption() {
    ...
    // Save Order
    await this._SalesOrderService.SaveDetails(this.orderModel).then(res => {
      if (res.succeeded == true) {
    ...
    if (this.orderModel.hasError == false) {
      await this._SalesOrderService.Approve(this.orderModel).then(res => {
        if (res.succeeded == true) {
          this.messageService.add({ severity: 'success', detail: this._AppMessageService.MESSAGE_OK });
        }
        else {

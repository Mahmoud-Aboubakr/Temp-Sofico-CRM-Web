...
namespace SofiForce.Worker
{
    public class Worker3 : BackgroundService
    {
        ...
        public Worker3(ILogger<Worker3> logger, IConfiguration configuration, IOrderService orderService)
        {
            _logger = logger;
        ...
        {
            //should be this
            //_logger.LogInformation("Worker3 Start at: {time}", DateTimeOffset.Now);
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    var worker = new Criteria<BOServiceWorker>()
                        .Add(Expression.Eq(nameof(BOServiceWorker.ServiceWorkerCode), "3"))
                ...
                {
                    //should be this 
                    //_logger.LogError("Worker 3 {error}: {time}",ex.Message, DateTimeOffset.Now);
        ...
        {
            //should be this
            //_logger.LogInformation("Worker3 Stoped at: {time}", DateTimeOffset.Now);
